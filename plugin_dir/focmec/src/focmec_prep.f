...
C		aemrgS, tp, ts.  Format is A4,F9.4,7F9.3 for each line.
...
C
	...
	do while (ierr.eq.0 .and. j.le.nsta)
	  ...
	  if (ierr .eq. 0) then
	    write(2,*) commnt(1:lenc(commnt))
	    read(commnt,'(A4,F9.4,7F9.3)') sta(j),dist(j),az(j),
	...
	do while (ierr .eq. 0)
	  ...
	  if (ierr .eq. 0) then
		...
		if (lc .gt. 6) then 
		  ...
		  commnt(39:39) = ' '
		  commnt(40:40+lc-10) = info(4:lc-6)
		  lc = 39 + lc - 9
		else
		  lc = 21
		end if
...
C+
	subroutine getrat(commnt,Pemrga,Semrga,tp,ts,vrat,pi,rddeg)
C
C	Starting in position 40, commnt has amplitudes, Q values, and
...
c-
	sense = commnt(21:21)
	lc = lenc(commnt)
	j1 = 39 + index(commnt(40:lc),',')
	if (j1 .le. 40) then
		write(*,*) 'No delimiter in ',commnt(40:lc)
		stop
	end if
	read(commnt(40:j1-1),*) anum
...
C	the critical angle, the results are unreliable.
C
	call grndmtn(rddeg,2,vrat,Semrga,Sar,Sav,pr,pv)
	angcrit = rddeg*asin(vrat)
	if (sense .eq. 'S') then
	  fsfact = 2.0/Sar
	  if (semrga .ge. angcrit) then
           write(*,*)'WARNING: SV emerge at or above critical angle'
	   write(2,*)'WARNING: SV emerge at or above critical angle'
	  end if
	else
	  call grndmtn(rddeg,1,vrat,Pemrga,Par,Pav,pr,pv)
	  if (sense .eq. 'V') then
	    fsfact = Pav/Sar
	    if (semrga .ge. angcrit) then
	     write(*,*)'WARNING: SV emerge at or above critical angle'
