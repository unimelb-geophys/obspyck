...
C
C	11 August 1985:  Added MOMTEN to argument of FMREPS
...
C	30 May 2002: Small changes to include fact that fmreps can allow
...
C-
	...
	INTEGER IPENS(3)
	LOGICAL CHOICE(3),FIRST,TRUTH,DATPLT,AN,PT,DSR,UPPER
	...
	DATA AN,PT,mt,DSR/3*.FALSE.,.TRUE./
C
	PI = 4.0*ATAN(1.0)
	FIRST = .TRUE.
...
400	IF(FILE) THEN    
	...
	ELSE
	  ...
	  READ(*,*) (ANGS(I),I=1,3)
	END IF
	CALL FMREPS(ANBTP,ANGS,PTTP,ANGS2,AN,PT,DSR,mt,MOMTEN,LUN1,LUN2)
	IF (PTB) THEN
...
500	IF (FILE) CLOSE(UNIT=1)
	...
	IF (TRUTH('Add more solutions to same plot?..[N]')) GO TO 100
	RETURN
	END
